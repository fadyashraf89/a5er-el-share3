...
class _DriverTripListState extends State<DriverTripList> {
      ...
      body: BlocConsumer<TripCubit, TripState>(
        ...
        builder: (context, state) {
          if (state is TripAccepted) {
            return _buildTripAccepted(state);
          } else if (state is TripStarted) {
            return _buildTripStarted(state, context);
  ...
  Center _buildTripStarted(TripStarted state, BuildContext context) {
    final trip = state.trip;
    return Center(
      child: Column(
  ...
  FutureBuilder<List<LatLng?>> _buildTripAccepted(TripAccepted state) {
    final trip = state.trip;
    return FutureBuilder<List<LatLng?>>(
      future: Future.wait([
        _convertAddressToLatLng(trip.FromLocation ?? "Unknown From"),
        _convertAddressToLatLng(trip.ToDestination ?? "Unknown To"),
      ]),
  ...
  Widget _buildActiveTrips() {
    final getActiveTripsStreamUseCase active =
        sl<getActiveTripsStreamUseCase>();
    ...
    return StreamBuilder<List<Trip>>(
      ...
      builder: (context, snapshot) {
        ...
        activeTrips = (snapshot.data ?? [])
            .where((trip) =>
                trip.Status == 'active' && !skippedTrips.contains(trip.id))
            .toList();
